...
async function main() {
  ...
  assert.equal(data.stats.vendorCount, 5, '厂商记忆数量错误');
  ...
  console.log(JSON.stringify({ vendorCount: data.stats.vendorCount, productCount: data.stats.productCount, healthDecision: result.results[0].decision }, null, 2));
}
