...
async function refreshState() {
  const [overview, vendors, products, documents, settings, runs, automation] = await Promise.all([api('/api/overview'), api('/api/vendors'), api('/api/products'), api('/api/documents'), api('/api/settings'), api('/api/runs'), api('/api/automation')]);
  Object.assign(state, { overview, vendors, products, documents, settings, runs, automation });
}
function mount(view) {
...
function renderUpdates() {
  const profiles = state.automation?.profiles || [];
  const profile = profiles.find(p => p.profileId === 'ale_omniswitch') || profiles[0];
  const worker = state.automation?.status?.worker || {};
  const runtime = profile ? (state.automation?.status?.profiles?.[profile.profileId] || {}) : {};
  const weekday = ['周日','周一','周二','周三','周四','周五','周六'];
  const target = document.getElementById('automation-control');
  if (!profile) { target.innerHTML = '<p class="form-error">未发现自动采集来源配置。</p>'; return; }
  const status = runtime.state || (profile.bootstrapComplete ? 'idle' : '待初始化');
  target.innerHTML = `<div class="automation-summary"><div class="detail-grid"><div class="detail"><span>受控来源</span><strong>${esc(profile.displayName)}</strong></div><div class="detail"><span>官方资料数</span><strong>${profile.sourceCount}</strong></div><div class="detail"><span>首次镜像</span><strong>${profile.bootstrapComplete ? '已完成' : '待执行'}</strong></div><div class="detail"><span>后台状态</span><strong>${esc(status)}</strong></div><div class="detail"><span>上次结果</span><strong>${esc(runtime.outcome || profile.lastOutcome || '尚未运行')}</strong></div><div class="detail"><span>上次完成</span><strong>${fmtDate(runtime.completedAt || profile.lastCompletedAt)}</strong></div></div><div class="setting-row automation-actions"><button id="run-automation" class="primary">立即运行 ALE OmniSwitch</button><span class="muted">请求将写入 NAS 本地队列；后台采集器只访问已登记的官方 HTTPS 来源。</span></div><div class="setting-row"><label class="toggle-label"><input id="automation-enabled" type="checkbox" ${profile.enabled ? 'checked' : ''} /> 启用此来源</label><label class="toggle-label"><input id="automation-schedule-enabled" type="checkbox" ${profile.schedule?.enabled ? 'checked' : ''} /> 启用每周计划</label><label>星期<select id="automation-weekday">${weekday.map((name,index)=>`<option value="${index}" ${Number(profile.schedule?.weekday)===index?'selected':''}>${name}</option>`).join('')}</select></label><label>时间<input id="automation-hour" type="number" min="0" max="23" value="${Number(profile.schedule?.hour ?? 2)}" /> : <input id="automation-minute" type="number" min="0" max="59" value="${Number(profile.schedule?.minute ?? 15)}" /></label><button id="save-automation" class="secondary">保存自动计划</button></div><p class="small muted">后台轮询：${worker.pollSeconds || '—'} 秒。计划只触发低频增量任务；没有元数据差异时不会重复下载 PDF。</p></div>`;
  document.getElementById('run-health-2').addEventListener('click',runHealthCheck);
  document.getElementById('run-automation').addEventListener('click', async (event) => { const button=event.currentTarget; button.disabled=true; button.textContent='已写入队列…'; try { const queued=await api(`/api/automation/profiles/${encodeURIComponent(profile.profileId)}/run`,{method:'POST',body:JSON.stringify({})}); await refreshState(); renderUpdates(); notify(`已请求自动采集：${queued.id}。后台任务将执行公开资料增量核验。`); } catch(err) { button.disabled=false; button.textContent='立即运行 ALE OmniSwitch'; notify(err.message,true); } });
  document.getElementById('save-automation').addEventListener('click', async () => { try { await api(`/api/automation/profiles/${encodeURIComponent(profile.profileId)}`, { method:'PUT', body:JSON.stringify({ enabled:document.getElementById('automation-enabled').checked, schedule:{ enabled:document.getElementById('automation-schedule-enabled').checked, weekday:Number(document.getElementById('automation-weekday').value), hour:Number(document.getElementById('automation-hour').value), minute:Number(document.getElementById('automation-minute').value) } }) }); await refreshState(); renderUpdates(); notify('NAS 本地自动采集计划已保存。'); } catch(err) { notify(err.message,true); } });
}
