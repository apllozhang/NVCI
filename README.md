...
| 新手任务向导（v1.2.0） | 以归档目标、厂商、产品线/系列来源、执行时间和归档字段目录组织采集任务；支持立即、一次预约、每日与每周执行，并将产品列表、产品页 URL、PDF URL、队列状态和待批准字段目录放在同一任务卡。 |
| 无模型公开资料采集器（v1.2.0） | 可通过 `npm run collect` 或独立技能复用已批准来源配置，执行确定性 PDF 增量采集、SHA-256 核验、快照与 manifest 归档；不调用 AI，不生成竞争结论。 |

> 当前发布版本：**v1.2.0**。P0-3 的字段事实门禁见 [`P03_ALE_CONTROLLED_FIELD_FACT_IMPORT.md`](./P03_ALE_CONTROLLED_FIELD_FACT_IMPORT.md)；P0-4 的关系状态、证据准入与验收标准见 [`P04_MODEL_LEVEL_RELATIONSHIP_LIBRARY.md`](./P04_MODEL_LEVEL_RELATIONSHIP_LIBRARY.md)；P0-4.1 的直接候选审阅建议、`4SFP`/`4SFP+` SKU 边界和人工批准要求见 [`P041_DIRECT_CANDIDATE_REVIEW_ADVISORY.md`](./P041_DIRECT_CANDIDATE_REVIEW_ADVISORY.md)。v1.2.0 的新手任务向导仅覆盖采集、整理和归档；其产品边界、状态机和门禁见 [`docs/onboarding-wizard-prd.md`](./docs/onboarding-wizard-prd.md)，无模型采集器契约见 [`docs/deterministic-collector-contract.md`](./docs/deterministic-collector-contract.md)。竞争对比分析属于下一阶段。
...
对于需要脱离 Web 页面、浏览器会话和 AI 模型复用的采集，可直接在 NVCI 项目中运行：

```bash
# 先预览已批准来源和固定输出资产；不会发起网络请求
npm run collect -- --data-dir /data --profile ale_omniswitch --dry-run

# 写入 NAS Worker 队列，浏览器关闭后继续执行
npm run collect -- --data-dir /data --profile ale_omniswitch --queue

# 当前命令串行执行增量采集
npm run collect -- --data-dir /data --vendor ale --product-line switches --run
```

脚本只接收已批准来源配置的筛选条件，不接受任意 URL。完整输入、输出、SHA-256 和失败处理契约见 [`docs/deterministic-collector-contract.md`](./docs/deterministic-collector-contract.md)。
...
# v1.1.0 继承 v0.9.1 的 Alpine 构建修复：better-sqlite3 强制从源码编译，避开 prebuild-install 的外部预编译包等待。
